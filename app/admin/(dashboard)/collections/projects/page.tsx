...
export default async function Page(props: PageProps) {
    ...
    const { data, error } = await supabase.from("projects").select("id, name, description, created_at").ilike('name', `%${searchParams?.name || ""}%`).order('created_at', { ascending: false });

    return (
        <>
            <div className="flex justify-between items-start">
                <CollectionHeader title="Projects" totalEntries={data?.length || 0} />
                <CreateFormDialog />
            </div>
