...
export async function createTalentStatus(prevState: FormState, formData: FormData): Promise<FormState> {
    ...
    try {
        ...
        const { error } = await supabase.from("talent_statuses").insert({ name, description })
...
export async function updateTalentStatus(id: string, prevState: FormState, formData: FormData): Promise<FormState> {
    // Extract and validate raw form entries using the schema
    const validatedFields = FormSchema.safeParse({
        name: formData.get('name'),
        description: formData.get('description')
...
export async function deleteTalentStatus(id: string) {
    try {
        const supabase = await createClient()
        const { error } = await supabase.from("talent_statuses").delete().eq('id', id)
        if (error) throw error
